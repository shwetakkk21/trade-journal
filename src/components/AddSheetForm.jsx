...
import { Link2, FolderOpen, Wand2 } from 'lucide-react';
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl">
      <h2 className="text-xs font-bold uppercase tracking-wider text-slate-300 mb-4 flex items-center gap-2">
        <Link2 className="w-4 h-4 text-teal-400" /> Pick Your Google Spreadsheet
      </h2>
      ...
      <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
        <div>
          ...
          >
            <FolderOpen className="w-3.5 h-3.5 text-amber-500" /> Browse Workspace File
          </button>
        </div>

        <div className="md:col-span-2">
          <label className="text-[10px] uppercase font-bold text-slate-400 font-mono block mb-1.5">Selected Target Directory</label>
          <div className="flex gap-2">
            ...
            >
              <Wand2 className="w-3.5 h-3.5" /> Link Sheets
            </button>
          </div>
        </div>
      </form>
